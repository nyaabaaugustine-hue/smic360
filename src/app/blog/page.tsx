...
export default function BlogPage() {
  ...
  return (
    <>
      ...
      {readPost && typeof window !== 'undefined' && createPortal(
        <div
        ...
        >
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', height: 220, overflow: 'hidden' }}>
              <img src={readPost.img} alt={readPost.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(transparent 40%, rgba(7,22,40,0.8))' }} />
              <button
                onClick={() => setReadPost(null)}
                style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.3)', color: '#fff', width: 36, height: 36, borderRadius: '50%', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
              >✕</button>
              <div style={{ position: 'absolute', bottom: 16, left: 24 }}>
                <span style={{ background: '#FFC107', color: '#071628', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: 1 }}>{readPost.category}</span>
              </div>
            </div>
            {/* Body */}
            <div style={{ padding: '28px 32px 36px', maxHeight: '55vh', overflowY: 'auto' }}>
              <div style={{ display: 'flex', gap: 12, fontSize: 12, color: '#5a7186', marginBottom: 14, alignItems: 'center' }}>
                <span>{readPost.date}</span>
                <span>·</span>
                <span>{readPost.readTime}</span>
              </div>
              <h2 style={{ fontFamily: 'Oswald,sans-serif', fontSize: 'clamp(20px,3vw,28px)', fontWeight: 700, color: '#071628', lineHeight: 1.2, marginBottom: 20 }}>{readPost.title}</h2>
              <div style={{ fontSize: 15.5, color: '#5a7186', lineHeight: 1.82, whiteSpace: 'pre-line' }}>{readPost.full}</div>
            </div>
            {/* Footer */}
            <div style={{ padding: '16px 32px 24px', borderTop: '1px solid #dce8f7', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button onClick={() => { setReadPost(null); setBookOpen(true); }} className="btn btn-primary" style={{ fontSize: 13 }}>Book A Consultation</button>
              <button onClick={() => setReadPost(null)} className="btn btn-outline" style={{ fontSize: 13 }}>Close Article</button>
            </div>
          </div>
        </div>,
        document.body
